...
stylesNumber = prompt("Apply Character Styles.\rMake sure you have predefined Character Styles. Script assigns styles to each paragraph of selected in palette order (Normal Style is not taken into account). Enter the number of styles / paragraphs:", 2);
for (i = 0; i < sel.length; i++) {
	if (sel[i].characters.length > 0) {
		for (j = 1; j <= stylesNumber; j++) {
			if (j <= sel[i].paragraphs.length){
				pp = sel[i].paragraphs[j-1];
				doc.characterStyles[j].applyTo(pp, true);
			}//end if j
		}//for j
	}//end if sel[i]
}//for i
